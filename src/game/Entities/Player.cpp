...
{
    auto factionEntry = sFactionStore.LookupEntry(faction);
...
void Player::RewardReputation(Creature* victim, float rate)
{
    ...
    if (Rep->repfaction1 && (!Rep->team_dependent || GetTeam() == ALLIANCE))
    {
        ...
        auto factionEntry1 = sFactionStore.LookupEntry(Rep->repfaction1);
        ...
        {
            auto team1_factionEntry = sFactionStore.LookupEntry(factionEntry1->GetTeam());
    ...
    if (Rep->repfaction2 && (!Rep->team_dependent || GetTeam() == HORDE))
    {
        ...
        auto factionEntry2 = sFactionStore.LookupEntry(Rep->repfaction2);
        ...
        {
            auto team2_factionEntry = sFactionStore.LookupEntry(factionEntry2->GetTeam());
...
void Player::RewardReputation(Quest const* pQuest)
{
    ...
    for (int i = 0; i < QUEST_REPUTATIONS_COUNT; ++i)
    {
        ...
        if (pQuest->RewRepValue[i])
        {
            ...
            if (auto factionEntry = sFactionStore.LookupEntry(pQuest->RewRepFaction[i]))
...
void Player::AddQuest(Quest const* pQuest, Object* questGiver)
{
    ...
    if (pQuest->GetRepObjectiveFaction())
        if (auto factionEntry = sFactionStore.LookupEntry(pQuest->GetRepObjectiveFaction()))
...
void Player::ReputationChanged(entry::view::FactionView factionEntry)
{
    for (int i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        if (uint32 questid = GetQuestSlotQuestId(i))
        {
            ...
            {
                if (qInfo->GetRepObjectiveFaction() == factionEntry->GetID())
                {
                    QuestStatusData& q_status = mQuestStatus[questid];
                    if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
                    {
