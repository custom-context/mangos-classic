...
GroupQueueInfo* BattleGroundQueue::AddGroup(Player* leader, Group* group, BattleGroundTypeId bgTypeId, BattleGroundBracketId bracketId, bool isPremade, uint32 instanceId)
{
    // create new ginfo
        ...
        if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
        {
            ...
            if (bgQueue.IsPlayerInvited(event.m_playerGuid, event.m_bgInstanceGuid, event.m_removeTime))
            {
                ...
                if (bg && bg->GetStatus() != STATUS_WAIT_LEAVE)
                    sBattleGroundMgr.ScheduleQueueUpdate(event.m_bgQueueTypeId, event.m_bgTypeId, bg->GetBracketId());
